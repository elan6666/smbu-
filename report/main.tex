...
\node[node] (intent) at (3.8, 0) {意图分类\\关键词兜底};
\node[smallnode] (table) at (8.0, 1.7) {结构化表查询\\分数/位次/证书/招生人数};
\node[smallnode] (rag) at (8.0, 0) {本地 RAG 检索\\招生章程/专业介绍};
\node[smallnode] (web) at (8.0, -1.7) {受控联网搜索\\官方域名优先};
\node[node] (evidence) at (12.0, 0) {证据融合\\来源片段 + 表格记录};
\node[smallnode] (qwen) at (15.9, 0.9) {Qwen 语言组织\\非结构化证据回答};
\node[smallnode] (template) at (15.9, -0.9) {规则模板\\保护数字和政策事实};
\node[node] (answer) at (19.8, 0) {带来源回答\\边界提示/风险说明};
...
\caption{系统在线问答流水线。用户问题首先进入意图分类模块，然后被路由到结构化招生表、本地 RAG 检索或受控联网搜索。系统将检索片段和结构化记录合并为证据包；非结构化证据回答由本地 Qwen 组织语言，分数线、证书、教学语言和招生人数等强事实回答由规则模板直接生成，最终输出带来源和边界说明的回答。}
